...
from Settings import c_input_buffer_size
...
def cnia(ser, conn, vessel):
    ...
    while f_cnia_repeat:
        ...
        output_buffer = bytearray([0x00])
        ser.write(output_buffer)
        # Give the arduino time to do its processing and return the data.
        while ser.in_waiting != c_input_buffer_size:  # Seems to work but needs a timeout/ error catch
            pass

        input_buffer = ser.read(c_input_buffer_size)
